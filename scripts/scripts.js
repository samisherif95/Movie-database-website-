$(document).ready(function(){
	...
	const nowPlayingURL = apiBaseURL + 'movie/now_playing?api_key=' + '09b7027887033a09fc28fd6b74973da1';
   
    const upComingURL = apiBaseURL + 'movie/upcoming?api_key=' + '09b7027887033a09fc28fd6b74973da1';
	...
	function getNowPlayingData(){
		$.getJSON(nowPlayingURL, function(nowPlayingData){
			...
			for(let i = 0; i<nowPlayingData.results.length; i++){
				...
				// mid = movie ID
				var thisMovieUrl = apiBaseURL+'movie/'+mid+'/videos?api_key=' + '09b7027887033a09fc28fd6b74973da1';
     ...
     function getUpComingData(){
		$.getJSON(upComingURL, function(UpComingData){
			//we needed to add .results because nowPlayingData is an array.// w300 is how wide it is
			for(let i = 0; i<UpComingData.results.length; i++){
                ...
                $.getJSON(thisMovieUrl, function(movieKey){
					...
					// added in i to nowPlayingHTML. Without it, only the details for the first movie in the results display in the modal no matter which movie poster you click on.
						...
						UpComingHTML += '<div class="rating">Rating: '+voteAverage+ '</div><br>';
					...
					//Without this line, there is nowhere for the posters and overviews to display so it doesn't show up 
					//$('#movieGenreLabel').html("Up Coming");
					//h1 will change depending on what is clicked. Will display "Up Coming" in this case.
				})
			}
		}) 
	}
	...
	function getMoviesByGenre(genre_id){
		const getMoviesByGenreURL = apiBaseURL + 'genre/' + genre_id + '/movies?api_key=' + '09b7027887033a09fc28fd6b74973da1' + '&language=en-US&include_adult=false&sort_by=created_at.asc';
	...
	$('.searchForm').submit(function(event){
		...
		//search term is only concerned with what the user inputed 
		//Get input with .val();
	...
	function searchMovies(){
		...
		const searchMovieURL = apiBaseURL + 'search/movie?api_key=09b7027887033a09fc28fd6b74973da1'  + '&language=en-US&page=1&include_adult=false&query=' + searchTerm;
			
		$.getJSON(searchMovieURL, function(movieSearchResults){
            
            if(movieSearchResults.results.length==0)
                $('#movieGenreLabel').html(" No results found ");
